...
import { CREATE_TASK, DELETE_TASK, ERROR_MESSAGE_TASK, GET_TASK, LOADING_MESSAGE_TASK, UPDATE_TASK, USER_INDIVIDUAL_TASK } from "./task.types";

export const getTask = (payload) => async (dispatch) => {
    try {
        dispatch({ type: LOADING_MESSAGE_TASK })
        const res = await axios.get(`https://muddy-garb-moth.cyclic.app/task/filter?sprintId=${payload}`);
...
export const createTask = (payload) => async (dispatch) => {
    ...
    try {
        dispatch({ type: LOADING_MESSAGE_TASK })
        let result = await axios.post('https://muddy-garb-moth.cyclic.app/task/', payload.form);
        console.log('result:', result.data)

        const res = await axios.get(`https://muddy-garb-moth.cyclic.app/task/filter?sprintId=${payload.sprintId}`);
...
export const deleteTask = (payload) => async (dispatch) => {
    try {
        dispatch({ type: LOADING_MESSAGE_TASK })
        await axios.delete(`https://muddy-garb-moth.cyclic.app/task/${payload.id}`);
...
export const updateTask = (payload) => async (dispatch) => {
    ...
    try {
        dispatch({ type: LOADING_MESSAGE_TASK })
        await axios.patch(`https://muddy-garb-moth.cyclic.app/task/${payload.id}`, payload.form);
...
export const userIndividualTask = (payload) => async (dispatch) => {
    ...
    try {
        dispatch({ type: LOADING_MESSAGE_TASK })
        let res = await axios.post(`https://muddy-garb-moth.cyclic.app/task/individual`, payload);
        console.log('res:', res)
